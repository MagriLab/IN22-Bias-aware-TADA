...
%         'k--','HandleVisibility','off');
    ...
    elseif Filter.E_Params == 2
        beta_train = ESN_P.training_beta;
        tau_train = ESN_P.training_tau;
%         figure
%         yyaxis left
        fn_plot_mean_std(t_KF, Aa_KF(:,end-1,:)/beta_train, ...
                        '$\beta (t) / \beta^\mathrm{train}$')
%         ylabel(['$\tilde{\beta}$ $\left[{\mathrm{W}}',...
%             '{\frac{\mathrm{s}^{1/2}}{\mathrm{m}^{5/2}}}\right]$'])

%         yyaxis right

        fn_plot_mean_std(t_KF, Aa_KF(:,end,:)/tau_train,...
                        '$\tau (t) / \tau^\mathrm{train}$')
%         ylabel('$\tilde{\tau}\left[s\right]$')
...
%     plot(t(Filter.init_BE_i)*[1,1],lims, 'k--','HandleVisibility','off');
    ...
    %
    t1          =   Filter.t_stop-0.2; [~,loc1]   =   min(abs(t_KF - t1));
    t2          =   Filter.t_stop+0.2; [~,loc2]   =   min(abs(t_KF - t2));
...
%     f_up    =    343/(1.18+0.74)/2;     f_down  =    589/(1.18+0.74)/2;
    ...
    ylim([min(actual_bias) max(actual_bias)]*1.02)
