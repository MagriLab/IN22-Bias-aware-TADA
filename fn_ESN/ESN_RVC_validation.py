...
exec(open(directory_path + "\\fn_ESN\\Functions.py").read())
...
N_washout = 50              # washout time = 50 * 0.0005 = 0.025 s
N_train   = 10*N_lyap        
...
with open(fln,'wb') as f:  # need 'wb' in Python3
    ...
    savemat(f, {"training_beta": BETA})
    savemat(f, {"training_tau": TAU})
