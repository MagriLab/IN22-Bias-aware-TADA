function dYdt = gov_eqns_dim_tau(t, y, P, law )
    ...
    % ================= Apply BCs to the advection eqns ================= %   
    ...
    % ================= Evaluate u(t-tau) and p(t-tau) ================== %
    X_tau   =   tau / tau_adv;      % [nondim length units]
    u_ftau  =   interp1(P.X, V2, X_tau,"spline","extrap");     % u_xf at t - tau
    
    if length(y) >= 2*N_m+2*N_c
        p_ftau  =   interp1(P.X, W2, X_tau,"spline"); % p_xf at t - tau
    end 
    ...
    elseif contains(law,'Q1')
        qdot	=   beta * (sqrt(abs(1 + u_ftau/u_0)) - sqrt(1)) ; 
    ...
    K_V        	=    - 2 / tau_adv * D_c * V2 ; % Chebyshev
    if length(y) >= 2*N_m+2*N_c
        K_W        	=    - 2 / tau_adv * D_c * W2 ; % 
    end
    if any(isnan(K_V))
        wtf=1;
    end
